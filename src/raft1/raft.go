...
type AppendEntriesReply struct {
	...
	// for optimization
	// send inconsistent log entry to reduce rejectioins
	LastLogIndex int
	LastLogTerm  int
}
...
func (rf *Raft) commitLogEntries() {
	for {
		...
		msg := raftapi.ApplyMsg{CommandValid: true, Command: command, CommandIndex: index}
		rf.applyCh <- msg
	}
}
